    ...
    <div className="max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2 bg-white p-4 rounded-lg shadow-md">
        ...
        <button 
            ...
            className={`p-3 rounded-md transition-colors ${isListening ? 'bg-red-500' : 'bg-blue-500 hover:bg-blue-600'}`}
        >
          {/* Microphone Icon */}
        ...
        <button type="submit" className="bg-green-500 text-white font-bold py-3 px-6 rounded-md hover:bg-green-600">
          Search
        </button>
      </form>
      
      {isListening && <p className="text-center text-blue-600 mt-4 font-semibold">Listening...</p>}
      ...
      <div className="mt-8">
        ...
        <div className="space-y-4">
          {results.map(tasker => (
            <div key={tasker.username} className="bg-white p-6 rounded-lg shadow-md flex items-center justify-between">
              ...
              <Link to={`/taskers/${encodeURIComponent(tasker.username)}`} className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">
                  View Profile
              </Link>
            </div>
          ))}
